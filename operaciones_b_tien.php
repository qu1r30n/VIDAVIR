<?php
	class op_b
	{
		public function crear_bd($nombre)
		{
			...
			$connection = mysqli_connect($arr_servername_VG[1],$arr_username_VG[1],$arr_password_VG[1]);
		...
		public function crear_tab($nombre_bd,$nom_tab,$arr_col,$arr_tip,$arr_cant,$col_indice,$arr_aut_inc)
		{
			$columnas="";
			include("var_g.php");
			$connection = mysqli_connect($arr_servername_VG[1],$arr_username_VG[1],$arr_password_VG[1],$nombre_bd);
		...
		{
			include("var_g.php"); 
			$conn=mysqli_connect($arr_servername_VG[1],$arr_username_VG[1],$arr_password_VG[1]);
			//-------------------------------------------------------
			$result = mysqli_query($conn,"SHOW TABLES FROM ".$arr_database_VG[1]." LIKE '".$tableName."';");
			mysqli_close($conn);
		...
		{
			include("var_g.php"); 
			$conn=mysqli_connect($arr_servername_VG[1],$arr_username_VG[1],$arr_password_VG[1],$arr_database_VG[1]);
			//-------------------------------------------------------
		...
		public function insertar($nom_tab,$arr_colum,$arr_valores)
		{
			...
			$conexion = mysqli_connect($arr_servername_VG[1],$arr_username_VG[1],$arr_password_VG[1],$arr_database_VG[1]);
			mysqli_query($conexion,$insertar);
			mysqli_close($conexion);
		}

		public function editar($nom_tab,$arr_colum,$arr_valores,$arr_colum2,$arr_compara,$arr_valores2,$op_log)
		{
			include("var_g.php"); 
			//- comando eidtar ------------------------------------------------------------------------------
			$cantidad_columnas=count($arr_colum);//son los que se canviaran
			$cantidad_valores=count($arr_valores);//son los que se cambiaran
			$cantidad_columnas2=count($arr_colum2);//son los que se comparara para cambiar  el arr_column
			$cantidad_compara=count($arr_compara);//son los que se comparara para cambiar  el arr_column
			$cantidad_valores2=count($arr_valores2);//
			$cantidad_op_log=count($op_log);

			//UPDATE `inventario` SET `CODIGO` = '123' WHERE `inventario`.`ID` = 1;//codigo sql para modificar
			...
			{ 
				...
				{
					$editar=$editar." ".$arr_colum2[$i]." ".$arr_compara[$i]." ".$arr_valores2[$i]." ".$op_log[$i];
				}
				else
				{
